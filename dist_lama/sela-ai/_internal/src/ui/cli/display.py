"""Antarmuka tampilan terminal (CLI).
...
class CLIDisplay:
    ...
    def _schedule_render(self):
        ...
        if self._loop and self._use_ansi and self.running:
            try:
                ...
                logging.getLogger(__name__).error(f"Gagal menjadwalkan render: {e}")
            ...
            def _on_done(t: asyncio.Task):
                ...
                if exc:
                    logging.getLogger(__name__).error(
                        f"Tugas render CLI bermasalah: {exc}", exc_info=exc
                    )

            task.add_done_callback(_on_done)
        except Exception as e:
            logging.getLogger(__name__).error(f"Gagal membuat tugas render: {e}", exc_info=True)

    async def _safe_render(self):
        """Render aman (dengan kunci)."""
    ...
    async def _render_dashboard(self, full: bool = False):
        ...
        # 构建状态行
        mode_text = "Otomatis" if self._dash_auto_mode else "Manual"
        conn_text = "Terhubung" if self._dash_connected else "Tidak terhubung"

        lines = [
            f"Status : {trunc(self._dash_status)}",
            f"Koneksi: {conn_text} | Mode: {mode_text}",
            f"Emosi  : {self._dash_emotion}",
            f"Dialog : {trunc(self._dash_text)}",
            f"Musik  : {trunc(self._dash_music) if self._dash_music else '—'}",
        ]
    ...
    def _read_line_raw(self) -> str:
        """Baca input mode mentah (mendukung UTF-8)."""
        try:
            import termios
            import tty
        except ImportError:
    ...
    def show_help(self):
        """Tampilkan bantuan."""
        self._dash_text = (
            "Perintah: r=rekam/berhenti | x=batal | q=keluar | "
            "h=bantuan | teks lain=dikirim sebagai pesan"
        )
        self._schedule_render()
        ...
        class DisplayLogHandler(logging.Handler):
            ...
            def emit(self, record: logging.LogRecord):
                try:
                    ...
                    logging.getLogger(__name__).error(f"Gagal mencatat log: {e}")
