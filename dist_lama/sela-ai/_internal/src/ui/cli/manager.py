...
class CliViewManager:
    ...
    async def start(self, mode: str = "cli"):
        ...
        """
        logger.info("CliViewManager: menjalankan antarmuka CLI...")
        self._running = True
        ...
        try:
            ...
            logger.info("CliViewManager: tugas tampilan dibatalkan")

    async def close(self):
        """Tutup tampilan CLI."""
        logger.info("CliViewManager: menutup...")
        self._running = False
        await self._display.close()
        logger.info("CliViewManager: ditutup")

    def _handle_command(self, cmd: str):
        """Tangani perintah pengguna - satu pintu masuk."""
        cmd_lower = cmd.lower()
    ...
    def _safe_emit(self, event: str, data=None):
        ...
        if self._task_manager is not None:
            try:
                ...
                logger.error(
                    f"CliViewManager gagal menjadwalkan event {event} lewat TaskManager: {e}",
                    exc_info=True,
                )
        ...
        try:
            ...
            def _done(f):
                ...
                if exc:
                    logger.error(
                        f"CliViewManager gagal memancarkan event {event}: {exc}",
                        exc_info=exc,
                    )

            fut.add_done_callback(_done)
        except Exception as e:
            logger.error(f"CliViewManager gagal menjadwalkan event {event}: {e}", exc_info=True)
