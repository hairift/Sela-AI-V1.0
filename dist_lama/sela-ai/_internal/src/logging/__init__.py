...
def setup_logging(
    ...
    # 控制台处理器
    # 打包后的 windowed 程序没有控制台（sys.stdout 为 None）。此前仍会创建
    # StreamHandler(None)，logging 回退到内部 lastResort 处理器，一旦日志含
    # 非 ASCII 字符就会在编码阶段抛 UnicodeEncodeError。这里直接跳过。
    if config.enable_console and sys.stdout is not None:
        # 控制台编码不是 UTF-8 时（Windows 默认 cp1252/cp936），中文或
        # 特殊字符会再次触发编码错误；尽量切换为 UTF-8 并容错。
        try:
...
def get_logger(name: str | None = None) -> logging.Logger:
    ...
    # Bila sistem log belum diinisialisasi, tambahkan handler konsol dasar.
    # Dilewati bila tidak ada konsol (mis. aplikasi windowed hasil paket),
    # agar logging tidak jatuh ke handler cadangan yang bisa gagal encoding.
    if (
        not _initialized
