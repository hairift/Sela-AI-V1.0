...
# Antarmuka QML (PySide6) kini bersifat opsional - SELA memakai antarmuka web
# sebagai bawaan. Uji yang menargetkan modul QML dilewati bila `gui` extra
# tidak terpasang, alih-alih gagal.
...
perlu_pyside6 = pytest.mark.skipif(
    not ADA_PYSIDE6,
    reason="PySide6 tidak terpasang (opsional: pip install -e '.[gui]')",
)
...
async def test_ui_plugin_routes_music_to_music_line():
    ...
    assert any("Sedang diputar" in t for t in vp.music)
    assert "[00:01/03:00] 第一句" in vp.music
    assert "你好小智" in vp.chat
    assert not any("Sedang diputar" in t or "第一句" in t for t in vp.chat)
