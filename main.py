...
def parse_args():
    ...
    # 运行模式选择
    # - web: 浏览器界面（SELA，默认；无需 PySide6，跨平台最稳）
    ...
    parser.add_argument(
        "--mode",
        choices=["web", "gui", "cli", "tui", "gpio"],
        default="web",
        help="运行模式（默认 web）：web / gui / cli / tui(全屏终端) / gpio(仅Linux)",
    )
    parser.add_argument(
        "--protocol",
    ...
    parser.add_argument(
        "--doctor",
        action="store_true",
        help="运行环境自检（音频/Opus/网络/配置）后退出，便于排查故障",
    )
    parser.add_argument(
        "--portrait",
        action="store_true",
        help="以竖屏（portrait）尺寸打开界面，适合 Raspberry Pi 触摸屏",
    )
    parser.add_argument(
        "--kiosk",
        action="store_true",
        help="以全屏 kiosk 模式打开界面（无边框，适合展示机/信息亭）",
    )
...
def _report_fatal(error: BaseException) -> None:
    """把致命错误写进崩溃日志，并在有图形界面时弹窗提示.

    这是为了修掉「Windows 上双击图标没反应」的常见根因：打包后的程序
    以 windowed 模式运行、没有控制台，异常被静默吞掉。现在错误一定会
    落到 ``logs/crash-*.log``，Windows 上还会弹出对话框指出日志位置。
    """
    import datetime
    import traceback
    ...
    log_path = None
    try:
        from src.utils.resource_finder import get_user_log_dir
    ...
    if sys.platform == "win32":
        try:
            ...
            ctypes.windll.user32.MessageBoxW(
                0, message, "SELA AI - Terjadi kesalahan", 0x10
            )
            return
        except Exception:
            pass
