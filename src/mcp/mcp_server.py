...
class McpServer:
    ...
    def add_common_tools(self, music_player=None, volume_controller=None):
        ...
        def _pasang(nama: str, fn, *args) -> None:
            """Daftarkan satu kelompok tool; kegagalan tidak menular ke yang lain.

            Sebelumnya seluruh blok berada dalam satu try yang sama, sehingga satu
            modul opsional yang gagal diimpor (mis. httpx untuk kamera VL) membuat
            SELURUH tool MCP hilang - termasuk musik dan pengetahuan kampus.
            """
            try:
                fn(*args)
            except Exception as e:
                logger.error(f"Gagal mendaftarkan tool MCP '{nama}': {e}", exc_info=True)

        if music_player is not None:
            try:
                from src.mcp.tools.music import register_music_tools

                _pasang("musik", register_music_tools, self.add_tool, music_player)
            except Exception as e:
                logger.error(f"Gagal memuat tool MCP 'musik': {e}", exc_info=True)

        try:
            from src.mcp.tools.app import register_app_tools

            _pasang("aplikasi", register_app_tools, self.add_tool)
        except Exception as e:
            logger.error(f"Gagal memuat tool MCP 'aplikasi': {e}", exc_info=True)

        try:
            from src.mcp.tools.kampus import register_kampus_tools

            # Pengetahuan kampus UCIC (RAG anti-halusinasi dari dokumen resmi).
            _pasang("kampus", register_kampus_tools, self.add_tool)
        except Exception as e:
            logger.error(f"Gagal memuat tool MCP 'kampus': {e}", exc_info=True)

        try:
            from src.mcp.tools.volume import register_volume_tools

            _pasang("volume", register_volume_tools, self.add_tool, volume_controller)
        except Exception as e:
            logger.error(f"Gagal memuat tool MCP 'volume': {e}", exc_info=True)
