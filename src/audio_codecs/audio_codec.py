...
class AudioCodec:
    ...
    def _output_callback(self, outdata, frames, time_info, status):
        ...
        try:
            ...
            # 通知输出监听器（界面 lipsync 等）。传单声道副本，且不在此线程
            # 写日志——输出回调对时间极其敏感。
            if self._output_listeners:
                try:
                    with self._listeners_lock:
                        listeners = list(self._output_listeners)
                    mono = np.asarray(outdata, dtype=np.float32)
                    if mono.ndim > 1:
    ...
    def add_output_listener(self, listener: Callable[[np.ndarray], None]):
        """添加输出（播放）监听器，接收最终混音 PCM。

        供界面 lipsync 使用：拿到的是真正送往扬声器的采样，因此嘴型
        与实际声音天然同步。回调在音频线程执行，必须**轻量且不阻塞**。

        Args:
            listener: 接收 float32 单声道 ndarray 的可调用对象
        """
        with self._listeners_lock:
            if listener not in self._output_listeners:
                self._output_listeners.append(listener)
                logger.info("已添加音频输出监听器（lipsync）")

    def remove_output_listener(self, listener: Callable[[np.ndarray], None]):
        """移除输出（播放）监听器（线程安全）."""
        with self._listeners_lock:
            if listener in self._output_listeners:
                self._output_listeners.remove(listener)
                logger.info("已移除音频输出监听器（lipsync）")
