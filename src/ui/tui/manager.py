...
class TuiViewManager:
    ...
    async def start(self, mode: str = "tui"):
        ...
        try:
            ...
            logger.error(
                "Mode TUI memerlukan textual. Pasang dengan:\n"
                "  uv sync --extra tui\n"
                "  pip install '.[tui]'\n"
                f"(galat asli: {e})"
            )
        ...
        logger.info("TuiViewManager: menjalankan antarmuka TUI...")
        self._running = True
        ...
        try:
            ...
            logger.info("TuiViewManager: tugas TUI dibatalkan")
            app = self._app
            if app is not None:
                try:
                    app.exit()
            ...
            if self._app is not None:
                try:
                    ...
                    pass
            logger.info("TuiViewManager: TUI berakhir")

    async def close(self):
        """关闭 TUI."""
        logger.info("TuiViewManager: menutup...")
        self._running = False
        app = self._app
        if app is not None:
            try:
            ...
            try:
                ...
                pass
        logger.info("TuiViewManager: ditutup")
    ...
    def _safe_emit(self, event: str, data=None):
        ...
        if self._task_manager is not None:
            try:
                ...
                logger.error(
                    f"TuiViewManager gagal menjadwalkan event {event} lewat TaskManager: {e}",
                    exc_info=True,
                )
        ...
        try:
            ...
            def _done(f):
                ...
                if exc:
                    logger.error(
                        f"TuiViewManager gagal memancarkan event {event}: {exc}",
                        exc_info=exc,
                    )

            fut.add_done_callback(_done)
        except Exception as e:
            logger.error(f"TuiViewManager gagal menjadwalkan event {event}: {e}", exc_info=True)
    ...
    def _set_app_attr(self, name: str, value) -> None:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"TUI gagal menetapkan {name}: {e}")
